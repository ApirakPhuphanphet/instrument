...
export const instrumentGroupRoutes = async (fastify) => {
    ...
    fastify.post('/instrument-groups', {
        ...
        try {
            const group = await instrumentGroupService.createGroup(request.body);
    ...
    fastify.get('/instrument-groups', {
        ...
        try {
            const result = await instrumentGroupService.getGroups(request.query);
    ...
    fastify.patch('/instrument-groups/:id', {
        ...
        try {
            const { id } = request.params;
            const group = await instrumentGroupService.updateGroup(id, request.body);
            return reply.status(200).send({
