...
export const instrumentRoutes = async (fastify) => {
    ...
    fastify.post('/instruments', {
        ...
        try {
            const instrument = await instrumentService.createInstrument(request.body);
    ...
    fastify.get('/instruments', {
        ...
        try {
            const result = await instrumentService.getInstruments(request.query);
    ...
    fastify.patch('/instruments/:id', {
        ...
        try {
            const { id } = request.params;
            const instrument = await instrumentService.updateInstrument(id, request.body);
            return reply.status(200).send({
