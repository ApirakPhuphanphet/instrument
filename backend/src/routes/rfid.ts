...
import {
  ...
  LoadResponseSchema,
  ListRfidsQuerySchema,
  CreateRfidSchema,
  DeleteRfidQuerySchema,
  ListRfidsResponseSchema,
  RfidBody,
  CheckRfidBody,
  CheckRfidQuery,
  LoadQuery,
...
export const rfidRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  fastify.get('/rfids', {
    ...
    try {
      const { search, type, status, page, limit, includeDeleted } = request.query as ListRfidsQuery;
      ...
      const data = records.map((r) => {
        ...
        let assignedType: 'staff' | 'instrument' | 'unassigned' = 'unassigned';
        if (user) assignedType = 'staff';
        else if (instrument) assignedType = 'instrument';
  ...
  fastify.post('/rfids', {
    ...
    try {
      const { id, type } = request.body as CreateRfidInput;
      const result = await prisma.rfid.upsert({
        where: { id },
        update: {
          type,
  ...
  const handleDeleteRfid = async (id: string, query: DeleteRfidQuery, reply: any) => {
    try {
  ...
  fastify.delete('/rfids/:id', {
    ...
    const query = request.query as DeleteRfidQuery;
