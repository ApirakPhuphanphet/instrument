...
export const userRoutes: FastifyPluginAsyncZod = async (fastify) => {
  ...
  fastify.get('/users', {
    ...
    try {
      const result = await userService.getUsers(request.query as UserQueryInput);
  ...
  fastify.get('/users/:id', {
    ...
    try {
      const { id } = request.params as UserParamsInput;
  ...
  fastify.patch('/users/:id', {
    ...
    try {
      const { id } = request.params as UserParamsInput;
      const user = await userService.updateUser(id, request.body as UpdateUserInput);
      return reply.status(200).send({
  ...
  fastify.delete('/users/:id', {
    ...
    try {
      const { id } = request.params as UserParamsInput;
      const { permanent } = request.query as DeleteUserQueryInput;
