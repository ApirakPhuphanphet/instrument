...
export class InstrumentGroupService {
   ...
   */
  async createGroup(data: CreateInstrumentGroupInput) {
   ...
   */
  async getGroups(query: InstrumentGroupQueryInput) {
   ...
   */
  async updateGroup(id: string, data: UpdateInstrumentGroupInput) {
    const group = await prisma.instrumentGroup.findUnique({
      where: { id }
    });
    ...
    if (group.deletedAt !== null) {
      throw new InstrumentGroupServiceError(
        `Cannot update deleted group '${id}'. Restore the group first.`,
        400
      );
    }
   ...
   */
  async deleteGroup(id: string, permanent = false) {
    const group = await prisma.instrumentGroup.findUnique({
      where: { id }
    });
